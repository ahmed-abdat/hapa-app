  ...
  switch (autoPopulate.mode) {
    ...
    case 'smart':
      ...
      if (!isPublishing && !isNewPost) {
        ...
        const looksAutoGenerated = currentDesc.includes('HAPA') || 
                                  currentDesc.includes('Haute Autorité') ||
                                  currentDesc.includes('الهيئة العليا')
        
        // Skip if not publishing and description doesn't look auto-generated
        if (!looksAutoGenerated && currentDesc.length > 50) {
          return data
        }
      }
      break
